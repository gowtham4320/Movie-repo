...
async function submitToServer(data: object) {
    try {
        const response = axios.post("https://www.melivecode.com/api/login", { ...data, expiresIn: 60000 })
    ...
    action$.pipe(
        ...
        mergeMap((action: any) =>
            from(submitToServer(action.payload)).pipe(
                map((res: any) => {
                    console.log(res);
                        Store.dispatch(snackBarText("Success",action.payload.expiresIn? "Login Success !":"Registration Success !"))
                ...
                catchError(error => of(error).pipe(
                    map((res:any)=>{
                        console.log(res)
                        Store.dispatch(snackBarText("Failure", action.payload.expiresIn ? "Login Failed !" : "Registration Failed !"))
                        return {
